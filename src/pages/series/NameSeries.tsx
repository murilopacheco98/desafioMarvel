...
import { selectById } from '../../store/modules/events/eventsSlice';

export const NameSeries: React.FC = () => {
  ...
  const eventReduxId = useAppSelector((state: any) =>
    selectById(state, id)
  );
    ...
    <>
      ...
      <ContainerCharacters>
        ...
        {eventReduxId ? (
          <>
            <ContainerTitle>
              <h1>{eventReduxId.creators.available}</h1>
            </ContainerTitle>
            <ContainerData>
              <ContainerImage>
                <ContainerCharacterId
                  image={`${eventReduxId.thumbnail.path}.${eventReduxId.thumbnail.extension}`}
                >
