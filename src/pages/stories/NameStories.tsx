...
import { selectById } from '../../store/modules/stories/storiesSlice';

export const NameStories: React.FC = () => {
  ...
  const storieReduxId = useAppSelector((state: any) =>
    selectById(state, id)
  );
    ...
    <>
      ...
      <ContainerCharacters>
        ...
        {storieReduxId ? (
          <>
            <ContainerTitle>
              <h1>{storieReduxId.creators.available}</h1>
            </ContainerTitle>
            <ContainerData>
              <ContainerImage>
                <ContainerCharacterId
                  image={`${storieReduxId.thumbnail.path}.${storieReduxId.thumbnail.extension}`}
                >
